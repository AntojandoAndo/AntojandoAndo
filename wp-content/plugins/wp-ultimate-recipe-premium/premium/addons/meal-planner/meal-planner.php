    ...
    public function get_select_options($recipe_groups)
    {
        ...
        foreach( $recipe_groups as $group )
        {
            ...
            foreach( $group['recipes'] as $recipe )
            {
                ...
                $nutritional = ' data-calories="' . $recipe->nutritional( 'calories' ). '"';
                $nutritional .= ' data-fat="' . $recipe->nutritional( 'fat' ). '"';
                $nutritional .= ' data-carbohydrate="' . $recipe->nutritional( 'carbohydrate' ). '"';
                $nutritional .= ' data-protein="' . $recipe->nutritional( 'protein' ). '"';

                $out .= '<option value="' . $recipe->ID() . '" data-servings="' . $servings . '" data-link="' . $recipe->link() . '" data-thumb="' . $recipe->image_url( 'thumbnail' ) . '"' . $nutritional . '>' . $recipe->title() . '</option>';
            }
